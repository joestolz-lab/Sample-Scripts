...
Set the server connection string in the variable below
Also, locate a valid run in the FDMEE env and set the run PID to the
other variable.
'''
...
conn = sql.DriverManager.getConnection(conexn1,c2,c3)

conn.setAutoCommit(False)
fdmAPI.initializeDevMode(conn)
p_process_id = BigDecimal(RunID)
fdmContext = fdmAPI.initContext(p_process_id)

batchName = "Location " + str(fdmContext["LOCNAME"])
fdmAPI.log( fdmContext["LOCNAME"])

# log the location of the instance. NEW
Msg = "Location of EPM " + fdmContext["EPMORACLEINSTANCEHOME"]
fdmAPI.log (Msg )
...
while(LocDetailsRS.next()):
  temp = LocDetailsRS.getString("PARTIMPGROUP")
  if ( temp == None) :
    temp="-blank-"
  t2 = LocDetailsRS.getString("PARTTARGETAPPLICATIONID")
  t3 = LocDetailsRS.getString("PARTVALGROUP")
  t4 = LocDetailsRS.getString("PARTDATAVALUE")
  if ( temp == None) :
    temp="-blank-"
  if ( t2 == None) :
    t2="-blank-"
  ...
  if ( t4 == None) :
    t4="-blank-"
  fdmAPI.log( LocDetailsRS.getString("PARTNAME")+ temp + t2 + t3 +t4);
LocDetailsRS.close

#ResultSet getRuleDetails(BigDecimal pRuleId) --which table is this going against?
p_ruleId = BigDecimal(50)
